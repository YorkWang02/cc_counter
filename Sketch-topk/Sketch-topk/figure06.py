...
for topk in topks:
    ...
    for index, func in enumerate(funcs):
        aae = [AAE[topk][mem][func] for mem in mems]
        print(aae)
        ax1.plot(range(len(mems)), aae, marker=markers[index], label=labels[func], linestyle='-',
    ...
    plt.grid(True, linestyle='--', axis='y')
    plt.grid(True, linestyle='--', axis='x')
    plt.ylabel(u'AAE', fontweight='bold', fontsize=10)
...
filepath = 'result/MergeCounter/800/aae.pdf'
plt.subplots_adjust(wspace=0.4, hspace=0.4, bottom=0.13, left=0.12, top=0.84)
