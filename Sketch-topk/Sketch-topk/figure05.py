...
for topk in topks:
    ...
    for index, func in enumerate(funcs):
        throughputs = [insertThroughput[topk][mem][func] for mem in mems]
        print(throughputs)
        ax1.plot(range(len(mems)), throughputs, marker=markers[index], label=labels[func], linestyle='-',
...
filepath = 'result/MergeCounter/800/throughput.pdf'
plt.subplots_adjust(wspace=0.4, hspace=0.4, bottom=0.13, left=0.12, top=0.84)
