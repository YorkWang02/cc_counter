...
private:
    ...
    //CM sketch
    BOBHash * bobhash_cm[MAX_HASH_NUM];
...
public:
	CCCounter4(uint _bucket,int _w, int _d) {
		bucket_num = _bucket;
		for (int i = 0; i < 1; i++) {
		...
		MAX_CNT = (1 << counter_index_size) - 1;
    ...
    double Query_CM(const char *str){
	...
	double Query(const char *key) {
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 1; j <= 4; j++) {
				if (bucket[i][hash[i]].fingerprint[j-1] == fp)
					return query(&bucket[i][hash[i]], j)*(MAX_CNT+1);
			}
		}
		return Query_CM(key);
	}
