...
public:
	CCCounter2(uint _bucket,uint32_t value) {
		bucket_num = _bucket;
        threshold = value;
		for (int i = 0; i < 2; i++) {
		...
		count = new int[MIN_HEAP_SIZE];
		memset(count, 0, sizeof(int) * MIN_HEAP_SIZE);
	...
	double Query(const char *key) {
        int sum = 0;    //统计过滤器中元素的频率
        for(int i = 0; i < pointer; i++){
			if(strcmp(key, items[i]) == 0){
                sum += count[i];
				// printf("test:i=%d,count=%d:\n",i,count[i]);
				// printf("test:the array usage rate:%d/%d\n",pointer+1,MIN_HEAP_SIZE);
            }		
		}
		// if(sum != 0)
		// {
        //     printf("test:query value:%d\n",sum);
		// 	return sum;
		// }
