...
for index, func in enumerate(keys):
    mems = [mem_func[0] for mem_func in average_ARE.keys() if mem_func[1] == func]
    are = [average_ARE[(mem, func)] for mem in mems]
    ax1.plot(mems, are, marker=markers[index], label=labels[func], linestyle='-',
             alpha=1, linewidth=2, markerfacecolor='none', zorder=105, color=colors[func])

plt.title('Average ARE', fontweight='bold', fontsize=12)
plt.xlabel('Memory (KB)', fontweight='bold', fontsize=10)
plt.ylabel('Average ARE', fontweight='bold', fontsize=10)
plt.grid(True, linestyle='--', axis='y')
plt.grid(True, linestyle='--', axis='x')
