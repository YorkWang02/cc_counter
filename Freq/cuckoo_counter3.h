...
private:
	struct bucket_t {	//one bucket
		char fingerprint[5];	//four fingerprints  
		uint8_t count12;		// 
...
public:
	...
	bool plus(int i, bucket_t* b, uint hash, int j) {		//try to plus entry_j in the bucket
										//return true if no overflow happens
		if (j == 1){
			b->count12++;
			insert_entry5(i,b,hash,j,b->count12 & 0xf);
		}
		else if (j == 2){
			b->count12 += 0x10;
			insert_entry5(i,b,hash,j,(b->count12 & 0xf0)>>4);
		}
		else if (j == 3){
			b->count3++;
			insert_entry5(i,b,hash,j,b->count3);
		}
		else if (j == 4){
			b->count4++;
			insert_entry5(i,b,hash,j,b->count4);
		}
		else if (j == 5) b->count5++;
	...
	void insert_entry5(int i, bucket_t* b, uint hash, int j,int count){
		uint rehashh = (hash ^ (bobhash[0]->run((const char*)&b->fingerprint[j-1], 1)));
		uint rehash = rehashh % bucket_num;

		//比较顺序为：本桶为空，备用桶为空，本桶可替换，备用桶可替换
		if(b->fingerprint[4] == NULL){
			b->fingerprint[4] = b->fingerprint[j-1];
			b->fingerprint[j-1] = NULL;
			b->count5 = count;
		}else if(bucket[1-i][rehash].fingerprint[4] == NULL){
			bucket[1-i][rehash].fingerprint[4] = b->fingerprint[j-1];
			b->fingerprint[j-1] = NULL;
			bucket[1-i][rehash].count5 = count;
		}else if(count > b->count5){
			char tmp_fp = b->fingerprint[j-1];
			b->fingerprint[j-1] = b->fingerprint[4];
			if(j == 1){
				b->count12 = (b->count12&0xf0)|b->count5;
			}else if(j == 2){
				b->count12 = (b->count12&0xf)|(b->count5<<4);
			}else if(j == 3){
				b->count3 = b->count5;
			}else{
				b->count4 = b->count5;
			}
			b->fingerprint[4] = tmp_fp;
			b->count5 = count;
		}else if(count > bucket[1-i][rehash].count5){
			char tmp_fp = bucket[1-i][rehash].fingerprint[4];
			int tmp_count = bucket[1-i][rehash].count5;
			bucket[1-i][rehash].fingerprint[4] = b->fingerprint[j-1];
			bucket[1-i][rehash].count5 = count;
			b->fingerprint[j-1] = NULL;
			if(tmp_count<0xf){
				if(tmp_count > (bucket[1-i][rehash].count12 & 0xf)){
					bucket[1-i][rehash].count12 = (bucket[1-i][rehash].count12 & 0xf0)|tmp_count;
					bucket[1-i][rehash].fingerprint[0] = tmp_fp;
				}else if(tmp_count > ((bucket[1-i][rehash].count12 & 0xf0)>>4)){
					bucket[1-i][rehash].count12 = (bucket[1-i][rehash].count12 & 0xf)|(tmp_count<<4);
					bucket[1-i][rehash].fingerprint[1] = tmp_fp;
				}else if(tmp_count > bucket[1-i][rehash].count3){
					bucket[1-i][rehash].count3 = tmp_count;
					bucket[1-i][rehash].fingerprint[2] = tmp_fp;
				}else if(tmp_count > bucket[1-i][rehash].count4){
					bucket[1-i][rehash].count4 = tmp_count;
					bucket[1-i][rehash].fingerprint[3] = tmp_fp;
				}
			}else if(tmp_count<0xff){
				if(tmp_count > bucket[1-i][rehash].count3){
					bucket[1-i][rehash].count3 = tmp_count;
					bucket[1-i][rehash].fingerprint[2] = tmp_fp;
				}else if(tmp_count > bucket[1-i][rehash].count4){
					bucket[1-i][rehash].count4 = tmp_count;
			...
			}else if(tmp_count<0xffff){
				if(tmp_count > bucket[1-i][rehash].count4){
					bucket[1-i][rehash].count4 = tmp_count;
					bucket[1-i][rehash].fingerprint[3] = tmp_fp;
				}
			}
	...
	void Insert(const char *key) {
		...
		//先遍历两个候选bucket的entry5
		for(int i=0; i<2; i++){
			if(bucket[i][hash[i]].fingerprint[4] == fp){
				plus(i,&bucket[i][hash[i]],hashh[i],5);
				return;
			}
		}
		//先在下面插入，再考虑替代entry5
		for (int i=0; i<2; i++) {
			for(int j=1; j<=4; j++) {
				if (bucket[i][hash[i]].fingerprint[j-1] == fp) {
					bool res = plus(i,&bucket[i][hash[i]],hashh[i], j);
		...
		if (flag) {
			...
			plus(ii,&bucket[ii][hash[ii]],hashh[ii] ,jj);
			return;
		}
		int i = fp&0x1;
