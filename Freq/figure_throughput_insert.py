...
for index, func in enumerate(keys):
    mems = [mem_func[0] for mem_func in average_insertThroughput.keys() if mem_func[1] == func]
    throughput_insert = [average_insertThroughput[(mem, func)] for mem in mems]
    ax1.plot(mems, throughput_insert, marker=markers[index], label=labels[func], linestyle='-',
             alpha=1, linewidth=2, markerfacecolor='none', zorder=105, color=colors[func])

plt.title('Average Throughput (Insert)', fontweight='bold', fontsize=12)
plt.xlabel('Memory (KB)', fontweight='bold', fontsize=10)
plt.ylabel('Average Throughput (Insert)', fontweight='bold', fontsize=10)
plt.grid(True, linestyle='--', axis='y')
plt.grid(True, linestyle='--', axis='x')
...
filepath = 'figure_throughput_insert.pdf'
