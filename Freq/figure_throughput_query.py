...
for index, func in enumerate(keys):
    mems = [mem_func[0] for mem_func in average_queryThroughput.keys() if mem_func[1] == func]
    throughput_query = [average_queryThroughput[(mem, func)] for mem in mems]
    ax1.plot(mems, throughput_query, marker=markers[index], label=labels[func], linestyle='-',
             alpha=1, linewidth=2, markerfacecolor='none', zorder=105, color=colors[func])

plt.title('Average Throughput (Query)', fontweight='bold', fontsize=12)
plt.xlabel('Memory (KB)', fontweight='bold', fontsize=10)
plt.ylabel('Average Throughput (Query)', fontweight='bold', fontsize=10)
plt.grid(True, linestyle='--', axis='y')
plt.grid(True, linestyle='--', axis='x')
...
filepath = 'figure_throughput_query.pdf'
