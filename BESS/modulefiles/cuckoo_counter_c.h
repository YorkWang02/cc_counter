...
private:
	...
	BOBHashcc * bobhashcc[2];		//Bob hash function

public:
	CCCounter(uint _bucket) {
		bucket_num = _bucket;
		for (int i = 0; i < 2; i++) {
			bobhashcc[i] = new BOBHashcc(i + 1000);
		}
	...
	void kickout(int loop, int i, bucket_t* b, uint8_t count, uint hash, int j) {
		...
		uint rehashh = (hash ^ (bobhashcc[0]->run((const char*)&b->fingerprint[j-1], 1)));
	...
	void Insert(const char *key) {
		...
		h1 = (bobhashcc[0]->run(key, strlen(key))); //% bucket_num;
		char fp = (char)(h1 ^ (h1 >> 8) ^ (h1 >> 16) ^ (h1 >> 24));
		h2 = (h1 ^ (bobhashcc[0]->run((const char*)&fp, 1))); //% bucket_num;		
